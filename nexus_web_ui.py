...
Uses background threading so Railway's HTTP timeout is never hit.
"""
...
# In-memory job store: { job_id: { status, path, error } }
JOBS: dict = {}
  ...
  <script>
    ...
    function pollJob(jobId) {
      ...
      pollInterval = setInterval(() => {
        elapsed += 3;
        const pct = Math.min(90, 5 + elapsed * 1.4);
        updateProgress(pct, getStatusMsg(elapsed));
...
def run_research(job_id: str, api_key: str, topic: str):
    try:
        # Temporarily patch the API key into research_agent
        original_key = ra.API_KEY
        ra.API_KEY = api_key
        ra.client = None  # force re-init if cached

        report = ra.research(topic)

        # Build PDF
        safe = ra.re.sub(r"[^\w\s-]", "", topic).strip().replace(" ", "_")[:50]
        ...
        with JOBS_LOCK:
            JOBS[job_id] = {"status": "done", "path": path, "topic": topic}

    except Exception as exc:
        with JOBS_LOCK:
            JOBS[job_id] = {"status": "error", "error": str(exc), "topic": topic}
    finally:
        try:
...
def start():
    ...
    with JOBS_LOCK:
        JOBS[job_id] = {"status": "running", "topic": topic}
