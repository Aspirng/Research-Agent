...
Rules: Every factual sentence ends with [n]. 4-6 sections. 5-8 real URLs from search only. Never fabricate URLs."""
    ...
    # Strategy 4: ask Claude to repair the JSON
    ...
    repair_resp = client.messages.create(
        ...
        messages=[{"role": "user", "content": f'Fix and return valid JSON with "executive_summary", "sections", and "sources" keys from this:\n\n{raw[:8000]}'}],
    )
    fixed = "".join(b.text for b in repair_resp.content if hasattr(b, "text")).strip()
    fixed = re.sub(r"```(?:json)?|```", "", fixed).strip()
    fs, fe = fixed.find("{"), fixed.rfind("}")
    if fs != -1 and fe != -1:
        fixed = fixed[fs:fe+1]
    try:
    ...
    raise ValueError("Could not parse research data from Claude. Please try again.")
